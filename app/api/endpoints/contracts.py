...
async def process_contracts(
    ...
    contratos: Optional[str] = Form(
        None, description="Lista de contratos separados por vírgula (vazio = todos)"
    ),
    service: ContractProcessingService = Depends(get_service),
):
...
async def get_job_status(
    job_id: str, service: ContractProcessingService = Depends(get_service)
):
