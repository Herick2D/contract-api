...
class ContractProcessingService:
    ...
    async def verify_pendencias(
        self, excel_path: Path, prints_dir: Optional[Path] = None
    ) -> Dict[str, Any]:
        try:
            import pandas as pd
            if prints_dir is None:
                prints_dir = self.prints_dir
            verificador = VerificadorPendencias(
    ...
    async def process_contracts(
        ...
        if not template_path.exists():
            ...
            return {"sucesso": False, "mensagem": "Arquivo do template não encontrado"}
        ...
        try:
            ...
            job["mensagem"] = (
                f"Processamento concluído: {job['sucessos']} sucessos, {job['falhas']} falhas"
            )
