...
class ExcelReader:
    ...
    def obter_contrato(self, numero: str) -> Optional[Contrato]:
        ...
        for i, nome in enumerate(nomes_inqs):
            inq = Inquilino(
                ...
                telefone=formatar_telefone(
                    tels_inqs[i]
                    if i < len(tels_inqs)
                    else (tels_inqs[0] if tels_inqs else "")
                ),
                email=(
                    emails_inqs[i]
                    if i < len(emails_inqs)
                    else (emails_inqs[0] if emails_inqs else "")
                ),
        ...
        enderecos_pps = separar_valores(
            limpar_texto(row_contatos.get("endereco_pps", ""))
        )
        ...
        for i, nome in enumerate(nomes_pps):
            prop = Proprietario(
                ...
                cpf=formatar_cpf(
                    cpfs_pps[i]
                    if i < len(cpfs_pps)
                    else (cpfs_pps[0] if cpfs_pps else "")
                ),
                rg=rgs_pps[i] if i < len(rgs_pps) else (rgs_pps[0] if rgs_pps else ""),
                email=(
                    emails_pps[i]
                    if i < len(emails_pps)
                    else (emails_pps[0] if emails_pps else "")
                ),
                telefone=formatar_telefone(
                    tels_pps[i]
                    if i < len(tels_pps)
                    else (tels_pps[0] if tels_pps else "")
                ),
                endereco=(
                    enderecos_pps[i]
                    if i < len(enderecos_pps)
                    else (enderecos_pps[0] if enderecos_pps else "")
                ),
            )
